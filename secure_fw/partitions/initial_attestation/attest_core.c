...
attest_add_profile_definition(struct attest_token_encode_ctx *token_ctx)
{
    ...
    if (size == 0) {
        ...
        WARN_UNPRIV("[Attest] Using a mismatched profile_definition received from the HAL\r\n");
    }
        ...
        int32_t *cose_algorithm_id)
{
    ...
    if (PSA_KEY_TYPE_IS_ECC(key_type) &&
        ...
        switch (psa_get_key_bits(&attr)) {
        case 128:
            *cose_algorithm_id = T_COSE_ALGORITHM_AES_CMAC_128;
            break;
        default:
            return PSA_ATTEST_ERR_GENERAL;
        }
...
#ifdef TFM_FIH_PROFILE_HIGH
/*!
 * \brief Size of the trailing token bytes that legitimately differ between two
 *        builds of the same token.
 *
 * The raw ECDSA signature is randomized, so it changes on every sign and sits
 * at the very end of the COSE_Sign1. MAC/CMAC tags are deterministic, so
 * nothing is excluded for symmetric attestation.
 */
static size_t attest_signature_tail_size(int32_t cose_algorithm_id)
{
                         ...
                         size_t *token_size)
{
    ...
    struct q_useful_buf_c completed_token;
#ifdef TFM_FIH_PROFILE_HIGH
    struct q_useful_buf verify;
    struct q_useful_buf_c verify_token;
    int32_t cose_algorithm_id;
    size_t sig_len;
    static uint8_t verify_buf[PSA_INITIAL_ATTEST_MAX_TOKEN_SIZE];
#endif /* TFM_FIH_PROFILE_HIGH */
    ...
    /* Fault-injection hardening: build the token a second time and require it
     * to match, ignoring the randomized ECDSA signature at the tail. */
    verify.ptr = verify_buf;
    verify.len = sizeof(verify_buf);
    attest_err = attest_create_token(&challenge, &verify, &verify_token);
    if (attest_err != PSA_ATTEST_ERR_SUCCESS) {
        goto error;
    }
    ...
    sig_len = attest_signature_tail_size(cose_algorithm_id);

    if (completed_token.len != verify_token.len) {
        attest_err = PSA_ATTEST_ERR_GENERAL;
        goto error;
    }

    (void)fih_delay();

    if ((completed_token.len < sig_len) ||
        (memcmp(completed_token.ptr, verify_token.ptr,
                completed_token.len - sig_len) != 0)) {
        attest_err = PSA_ATTEST_ERR_GENERAL;
        goto error;
    }
#endif /* TFM_FIH_PROFILE_HIGH */

...
static void mailbox_clean(void)
{
    struct mailbox_status_t *ns_status = spe_mailbox_queue.ns_status;
    ...
    for (uint8_t idx = 0; idx < spe_mailbox_queue.ns_slot_count; idx++) {
        mailbox_queue_status_t mask_bit = 1 << idx;
        if ((pend_slots & mask_bit) != 0) {
            /*
	     * Reject this new request
	     * PSA_ERROR_CONNECTION_BUSY indicates a temporary rejection
	     */
            mailbox_direct_reply(idx, (uint32_t)PSA_ERROR_CONNECTION_BUSY);
        } else if (!get_spe_queue_empty_status(idx)) {
            /*
	     * Reject this in-flight request
	     * PSA_ERROR_CONNECTION_BUSY indicates a temporary rejection
	     */
            mailbox_direct_reply(idx, (uint32_t)PSA_ERROR_CONNECTION_BUSY);
            pend_slots |= mask_bit;
    ...
    set_nspe_queue_replied_status(ns_status, pend_slots);
...
static int32_t tfm_mailbox_enable(void)
{
    int32_t ret;
...
static int32_t tfm_mailbox_disable(void)
{
    int32_t ret;

    /* discard any in-progress requests */
    mailbox_clean();

    ret = tfm_mailbox_hal_deinit(&spe_mailbox_queue);
    if (ret != MAILBOX_SUCCESS) {
        tfm_rpc_unregister_ops();
...
static int32_t tfm_mailbox_deinit(void)
{
    tfm_rpc_unregister_ops();

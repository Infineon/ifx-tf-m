...
TFM_COVERITY_DEVIATE_BLOCK(MISRA_C_2023_Rule_9_3, "Unused vector table entries are intentionally zero-initialized by the startup table design")
...
TFM_COVERITY_BLOCK_END(MISRA_C_2023_Rule_9_3)
...
__NO_RETURN void Reset_Handler(void)
{
    ...
    /* Enter via a volatile pointer and trap on return, so a fault that
     * skips the branch cannot run past Reset_Handler before isolation is set. */
    void (* volatile program_start)(void) = __PROGRAM_START;
    program_start();                          /* Enter PreMain (C library entry point) */
    for (;;) {
    }
}

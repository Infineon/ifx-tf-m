...
FIH_RET_TYPE(enum tfm_hal_status_t) ifx_sau_cfg(void)
{
    ...
    */
    ...
    for(idx = 0UL; idx < CY_SAU_REGION_CNT; idx++) {
        ...
        TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_18_1, "idx is bounded by CY_SAU_REGION_CNT which is the element count of SAU_config")
        if ((SAU_config[idx].base_addr & ~SAU_RBAR_BADDR_Msk) != 0U) {
            tfm_core_panic();
        }
        TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_18_1, "idx is bounded by CY_SAU_REGION_CNT which is the element count of SAU_config")
        if ((SAU_config[idx].size & ~SAU_RLAR_LADDR_Msk) != 0U) {
            tfm_core_panic();
        }

        /* Non-Secure Callable region is configured by TFM only */
        TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_18_1, "idx is bounded by CY_SAU_REGION_CNT which is the element count of SAU_config")
        if (SAU_config[idx].nsc) {
            tfm_core_panic();
        }
    ...
    if (((uint32_t)&REGION_NAME(Image$$, ER_VENEER, $$Base) & ~SAU_RBAR_BADDR_Msk) != 0U) {
        tfm_core_panic();
    }
    if (((uint32_t)&REGION_NAME(Image$$, VENEER_ALIGN, $$Limit) & ~SAU_RLAR_LADDR_Msk) != 0U) {
        tfm_core_panic();
    }
...
FIH_RET_TYPE(enum tfm_hal_status_t) ifx_sau_verify_static_boundaries(void)
{
    ...
    /* Verify that the SAU is actually enabled. ENABLE must be set and ALLNS must
     * remain clear so that the SAU never attributes the whole address space as
     * Non-secure. */
    if ((SAU->CTRL & (SAU_CTRL_ENABLE_Msk | SAU_CTRL_ALLNS_Msk)) != SAU_CTRL_ENABLE_Msk) {
        FIH_RET(TFM_HAL_ERROR_GENERIC);
    }

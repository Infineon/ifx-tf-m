...
/* Redundant copy of active_boundary, kept in sync with every update, used to
 * detect a fault that corrupts the domain gate in tfm_hal_memory_check. */
static uintptr_t active_boundary_check  = IFX_SPM_BOUNDARY;
                                              ...
                                              uint32_t asset_cnt)
{
    ...
    for (asset_idx = 0; asset_idx < asset_cnt; asset_idx++) {
        if (IFX_ASSET_ATTR_IS_SET(p_assets[asset_idx].attr, ASSET_ATTR_NAMED_MMIO)) {
            ...
            if (!IFX_ASSET_ATTR_IS_SET(p_assets[asset_idx].attr, ASSET_ATTR_READ_WRITE)) {
            ...
            if (IFX_ASSET_ATTR_IS_SET(p_assets[asset_idx].attr, ASSET_ATTR_READ_ONLY) ==
                IFX_ASSET_ATTR_IS_SET(p_assets[asset_idx].attr, ASSET_ATTR_READ_WRITE)) {
     ...
     * saving some memory. Also this improves run time performance */
            ...
            TFM_COVERITY_BLOCK_END(MISRA_C_2023_Rule_10_4 MISRA_C_2023_Directive_4_6)
        }
    }
...
fih_ret tfm_hal_verify_static_boundaries(void)
{
    ...
    TFM_COVERITY_BLOCK_END(MISRA_C_2023_Rule_15_6 MISRA_C_2023_Rule_10_4)
...
#ifdef TFM_FIH_PROFILE_ON
    /* Re-check the domain gate against the redundant copy. */
    (void)fih_delay();
    if ((active_boundary != active_boundary_check) ||
        (p_info->ifx_domain != IFX_GET_BOUNDARY_DOMAIN(active_boundary_check))) {
        tfm_core_panic();
    }
#endif /* TFM_FIH_PROFILE_ON */
#endif
...
FIH_RET_TYPE(enum tfm_hal_status_t) tfm_hal_post_partition_init_hook(void)
{
    ...
    IFX_SET_SPM_STATE(IFX_SPM_STATE_RUNNING);
...
FIH_RET_TYPE(bool) tfm_hal_boundary_need_switch(uintptr_t boundary_from, uintptr_t boundary_to)
{
    ...
    bool need_switch = (domain_from != domain_to);
#ifdef TFM_FIH_PROFILE_ON
    /* Recompute the decision from the complements after a delay so a glitch on
     * the single comparison cannot silently skip the boundary switch. */
    volatile uintptr_t inv_from = ~domain_from;
    volatile uintptr_t inv_to   = ~domain_to;
    (void)fih_delay();
    ...
    TFM_COVERITY_DEVIATE_LINE(cert_exp30_c, "Volatile variables do not have side-effects, so read order doesn't matter")
    if (need_switch != (inv_from != inv_to)) {
        tfm_core_panic();
    }
                                                ...
                                                uintptr_t boundary)
{
    ...
    */
    ...
    TFM_COVERITY_BLOCK_END(MISRA_C_2023_Rule_10_1 MISRA_C_2023_Rule_15_6 MISRA_C_2023_Rule_10_4)
    ...
    if ((boundary != IFX_SPM_BOUNDARY) && (boundary != active_boundary)) {
        ...
        TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_20_7, "Cannot wrap with parentheses due to Fault injection architecture and define FIH_RET_TYPE")
        FIH_CALL(ifx_mpu_disable_asset_regions, result);
        TFM_COVERITY_DEVIATE_BLOCK(MISRA_C_2023_Rule_10_4, "Cannot change types due to Fault injection architecture")
        TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_10_1, "Cannot change not equal logic due to Fault injection architecture and define FIH_NOT_EQ")
        if (FIH_NOT_EQ(result, TFM_HAL_SUCCESS)) {
            FIH_RET(result);
...
#ifdef TFM_FIH_PROFILE_ON
        (void)fih_delay();
        active_boundary_check = boundary;
#endif /* TFM_FIH_PROFILE_ON */
    } else {
        TFM_COVERITY_DEVIATE_BLOCK(MISRA_C_2023_Rule_10_4, "Cannot change types due to Fault injection architecture")
        TFM_COVERITY_DEVIATE_BLOCK(MISRA_C_2023_Rule_15_6, "Definition of FIH_SET() is missing {}, but FIH_PANIC is always a simple statement")
...
#ifdef TFM_FIH_PROFILE_ON
    /* Confirm the privilege level was actually applied */
    (void)fih_delay();

    uint32_t expected_npriv;
    if (boundary != IFX_SPM_BOUNDARY) {
        const ifx_partition_info_t *p_info = (const ifx_partition_info_t *)boundary;
        TFM_COVERITY_DEVIATE_BLOCK(MISRA_C_2023_Directive_4_6, "FIH uses basic types without size and signedness")
        TFM_COVERITY_DEVIATE_BLOCK(MISRA_C_2023_Rule_10_4, "Cannot change types due to Fault injection architecture")
        TFM_COVERITY_DEVIATE_LINE(MISRA_C_2023_Rule_10_1, "Cannot change equal logic due to Fault injection architecture and define FIH_EQ")
        expected_npriv = (IFX_IS_PARTITION_PRIVILEGED(p_info)) ? 0U : 1U;
        TFM_COVERITY_BLOCK_END(MISRA_C_2023_Rule_10_4 MISRA_C_2023_Directive_4_6)
    } else {
        /* SPM thread requires privileged access rights */
        expected_npriv = 0U;
    ...
    CONTROL_Type ctrl_check;
    ctrl_check.w = __get_CONTROL();
    if (ctrl_check.b.nPRIV != expected_npriv) {
        FIH_RET(TFM_HAL_ERROR_GENERIC);
    }
#endif /* TFM_FIH_PROFILE_ON */

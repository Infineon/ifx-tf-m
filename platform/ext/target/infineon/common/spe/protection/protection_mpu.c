                                         ...
                                         const struct mpu_armv8m_region_cfg_t *mpu_config)
{
    ...
    if (((mpu_config->region_base) & ~MPU_RBAR_BASE_Msk) != 0U) {
        tfm_core_panic();
    }
    /* As region limit is last address of region, it's necessary to add 1 to check alignment */
    if (((mpu_config->region_limit + 1U) & ~MPU_RLAR_LIMIT_Msk) != 0U) {
        tfm_core_panic();
    }
    /* Verify that base < limit */
    if (mpu_config->region_base >= mpu_config->region_limit) {
        tfm_core_panic();
    }
    ...
    /* Read back the region registers to ensure it was programmed as intended */
    (void)fih_delay();
...
{
    FIH_RET_TYPE(enum tfm_hal_status_t) fih_rc;
    ...
    */
    ...
    FIH_CALL(ifx_mpu_disable_asset_regions, fih_rc);
...
FIH_RET_TYPE(enum tfm_hal_status_t) ifx_mpu_disable_asset_regions(void)
{
    ...
    /* Read back to confirm every asset region was actually disabled */
    for(uint32_t idx = ifx_mpu_static_config_count; idx < CPUSS_CM33_0_MPU_S_REGION_NR; idx++) {
        MPU->RNR = _VAL2FLD(MPU_RNR_REGION, idx);
                                                    ...
                                                    const struct asset_desc_t *p_asset)
{
    ...
    if (IFX_ASSET_ATTR_IS_SET(p_asset->attr, ASSET_ATTR_READ_WRITE)) {
...
void ifx_mpu_shared_metadata_rw_enable(bool enable)
{
    ...
    */
    ...
    if ((MPU->RBAR & MPU_RBAR_BASE_Msk) != ((uint32_t)&REGION_NAME(Image$$,
                                                                   TFM_SPM_RW_OTHER_RO_START,
                                                                   $$Base)
                                            & MPU_RBAR_BASE_Msk)) {
        tfm_core_panic();
    }
    if ((MPU->RLAR & MPU_RBAR_BASE_Msk) != (((uint32_t)&REGION_NAME(Image$$,
...
#if defined(TFM_FIH_PROFILE_ON)
    /* Read back the permission to ensure the write window actually changed state */
    (void)fih_delay();
    if (_FLD2VAL(MPU_RBAR_AP, MPU->RBAR) != (uint32_t)permission) {
        tfm_core_panic();
    }
#endif /* TFM_FIH_PROFILE_ON */
}

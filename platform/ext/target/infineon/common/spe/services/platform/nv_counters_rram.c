...
*                   TFM_PLAT_ERR_CORRUPTION_DETECTED in case of incorrect checksum
                                                              ...
                                                              bool is_backup)
{
    ...
    if (!ifx_rram_counter_is_checksum_valid(&counter, counter_addr)) {
        return TFM_PLAT_ERR_CORRUPTION_DETECTED;
    }
...
static enum tfm_plat_err_t ifx_rram_recover_tfm_nv_counters(void)
{
    ...
    for (uint32_t counter_idx = 0UL; counter_idx < (uint32_t)PLAT_NV_COUNTER_MAX; counter_idx++) {
        ...
        status_backup = ifx_rram_read_nv_counter_or_backup((enum tfm_nv_counter_t)counter_idx,
                                                           &backup_val,
                                                           true);
        if ((status_backup != TFM_PLAT_ERR_SUCCESS) && (status_backup != TFM_PLAT_ERR_CORRUPTION_DETECTED)) {
            /* Unexpected error */
        ...
        status_counter = ifx_rram_read_nv_counter_or_backup((enum tfm_nv_counter_t)counter_idx,
                                                            &counter_val,
                                                            false);
        if ((status_counter != TFM_PLAT_ERR_SUCCESS) && (status_counter != TFM_PLAT_ERR_CORRUPTION_DETECTED)) {
            /* Unexpected error */
        ...
        if (status_backup == TFM_PLAT_ERR_SUCCESS) {
            ...
            if (status_counter == TFM_PLAT_ERR_SUCCESS) {
                 ...
                 * Repair the backup from the valid counter now to restore 2-of-2 redundancy;
                 * a counter that is never written again would otherwise remain single-copy. */
...
enum tfm_plat_err_t tfm_plat_init_nv_counter(void)
{
    ...
    /* The init-done flag gates a destructive clear of every counter (including the
     * anti-rollback counters), so the "not initialized" verdict is confirmed on both
     * the flag word and its complement: a single fault on one comparison cannot
     * trigger the mass clear. */
    volatile uint32_t flag     = counter.data.value;
    volatile uint32_t flag_inv = ~counter.data.value;

    if ((flag != IFX_NVM_INIT_DONE_FLAG) &&
        (flag_inv != ~(uint32_t)IFX_NVM_INIT_DONE_FLAG)) { /* NV Counters were not initialized */
        enum tfm_plat_err_t status;

        counter.data.value = IFX_NVM_INIT_DONE_FLAG;
        counter.checksum   = ifx_se_crc32d6a(sizeof(counter.data.bytes),
                                             counter.data.bytes, counter_addr);

        /* Write the init done flag before clearing the counters, so there is never a state where
         * the counters are valid but the flag is unset. Otherwise a glitched/skipped flag write
         * would leave the counters usable while later re-entering this path would clear (roll back)
         * already incremented counters. The flag is written again after clearing as extra hardening. */
        ret = IFX_NV_COUNTERS_CMSIS_FLASH_INSTANCE.ProgramData(counter_addr, &counter, cnt);
        ...
        status = ifx_rram_clear_counters_and_backups();
        if (status != TFM_PLAT_ERR_SUCCESS) {
            return status;
        }
        ...
        /* Verify the flag write so a torn or faulted program is not mistaken for a
         * completed initialization on the next boot. */
        ifx_rram_counter_t verify = {0};
        ret = IFX_NV_COUNTERS_CMSIS_FLASH_INSTANCE.ReadData(counter_addr, &verify, cnt);
        if ((ret < 0) || (ret != (int32_t)cnt) ||
            !ifx_rram_counter_is_checksum_valid(&verify, counter_addr) ||
            (verify.data.value != IFX_NVM_INIT_DONE_FLAG)) {
            return TFM_PLAT_ERR_SYSTEM_ERR;
        }
    }

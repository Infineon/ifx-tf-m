...
enum tfm_plat_err_t tfm_plat_get_implementation_id(uint32_t *size, uint8_t  *buf)
{
    ...
    uint8_t device_id_mpn[sizeof(IFX_NON_SECURE_SFLASH_BASE->DEVICE_ID_MPN)
                          - IFX_ASSETS_RRAM_CHECKSUM_SIZE];
    const uint32_t impl_id_size = sizeof(die_id) + sizeof(device_id_to) + sizeof(device_id_mpn);
    ...
    const uint32_t impl_id_size = sizeof(IFX_SFLASH->SI_REVISION_ID) + sizeof(IFX_SFLASH->SILICON_ID) +
...
#error "Unsupported implementation or invalid configuration"
    ...
    /* Update caller's size with the exact size of the implementation ID */
    *size = impl_id_size;

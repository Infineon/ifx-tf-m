    ...
    uint8_t *output, size_t output_size, size_t *output_length)
{
    ...
    *output_length = 0;
    ...
    /* Get Random bytes */
    status = ifx_se_get_tfm_psa_status( ifx_se_generate_random(
    ...
    if (status == PSA_SUCCESS) {
        ...
        if (crc == IFX_CRC32_CALC(random_buf, output_size)) {
            ...
            /* Fault-injection mitigation for a skipped or truncated output
             * copy: a single-instruction fault can skip the memcpy above (or
             * the CRC check) while execution still falls through to reporting
             * success, leaving the caller buffer stale. Recompute the CRC over
             * the destination and report success only when the delivered bytes
             * match the SE-provided CRC. */
            if (crc == IFX_CRC32_CALC(output, output_size)) {
    ...
    /* Clear the scratch buffer of live SE entropy before returning it to the
     * heap, so no confidential random bytes remain in reclaimable memory. */
    mbedtls_platform_zeroize(random_buf, output_size + IFX_CRC32_CRC_SIZE);

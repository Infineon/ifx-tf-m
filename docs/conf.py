...
copyright = '2017-2026, ARM CE-OSS, Infineon'
author = 'ARM CE-OSS, Infineon'
title = 'User Guide'
...
try:
    ...
    # Use the version from cmake if git command fails
    with open('../cmake/version.cmake', 'r') as f:
        for line in f:
            match = re.match(r'set\(TFM_VERSION_MANUAL\s+"(?P<TFM_VERSION_GROUP>.+?)"\)', line.strip())
            if match:
                version = release = match.group('TFM_VERSION_GROUP')
                break
